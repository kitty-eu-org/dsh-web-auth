...
export class WebServer extends Service {
  ...
  async [Service.init](): Promise<void> {
    const handle = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      ...
      if (gate !== undefined && !(await gate(req, res, 'request'))) return
    ...
    this.server.on('upgrade', (req, socket, head) => {
      ...
      try {
        Promise.resolve(gate(req, upgradeGateResponse(socket), 'upgrade')).then((allowed) => {
          if (allowed) this.dispatchUpgrade(req, socket, head)
        }, (error: unknown) => {
 ...
 */
function upgradeGateResponse(socket: Duplex): WebGateResponse {
  ...
  return {
    writeHead(statusCode, headers = {}) {
      ...
      for (const [name, value] of Object.entries(headers)) {
        for (const part of Array.isArray(value) ? value : [value]) headerLines.push(`${name}: ${part}`)
      }
    },
